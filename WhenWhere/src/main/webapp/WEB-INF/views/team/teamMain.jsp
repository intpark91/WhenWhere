		...
		<div class="content-wrapper">
			...
			<div class="row chatTable">
					...
					<div class="col-sm-12 teamList">
						<div id="roomListDiv" class="box box-default">
							<div class="box-header">
								<h3 class="box-title">
									<span> 팀 목록을 선택하면 <b style="color: red;">상세보기</b>를 할 수
										있어요
									</span>
								</h3>
							</div>
						...
						<div class="box box-primary">
							<div class="box-body box-profile">
								<img class="profile-user-img img-responsive img-circle"
									src="../images/team/profile.png" alt="User profile picture">
								...
								<ul class="list-group list-group-unbordered">
									...
									<li class="list-group-item"><b>팀 URL</b> <a> <b
											class="pull-right" id="a_url"></b>
									</a></li>
									<li class="list-group-item"><b>팀 상태</b> <a id="a_sts"
										class="pull-right"> </a></li>
									<li class="list-group-item"><b>팀원 목록</b> <a id="a_list"
										class="pull-right"> </a></li>
									<li class="list-group-item"><b>팀 유형</b> <a id="a_type"
										class="pull-right"> </a></li>
								</ul>

								<div class="col-sm-12 ">
									<div class="col-sm-6 join-btn">
										<a href="#" class="btn btn-primary btn-block btn-type"><b>
												선택된 팀 없음 </b></a>
									</div>
									<div class="col-sm-6">
										<a href="#" class="btn btn-primary btn-block applyUserListBtn"
											data-toggle="modal" data-target="#basicModal"><b>팀원
												관리하기</b></a>
									</div>
								</div>
							</div>
				...
				<div class="col-sm-6">
					<div id="chattingRoom">
						<div
							class="ddo-chat box box-primary direct-chat direct-chat-primary collapsed-box">
							<div class="box-header with-border">
							...
							<div class="box-footer">
								<form action="#" method="post">
									<div class="input-group">
										<input id="sendMsg" type="text" name="message"
											placeholder="Type Message ..." class="form-control">
										<span class="input-group-btn">
											<button type="button" onclick="sendMsg();"
												class="btn btn-primary btn-flat">메세지 보내기</button>
										</span>
									</div>
								</form>
							</div>
						...
						<div class="teamList">
							<div id="roomListDiv" class="box box-default">
								...
								<div class="box-body table-responsive no-padding">
									...
									<div class="box-footer clearfix">
										<a href="#" class="btn btn-primary applyUserDeleteBtn">선택
											삭제</a>
									</div>
								</div>
								<!-- /.box-body -->
	...
	<script type="text/javascript">
			...
			$(".mainTr")
					.on(
							...
							function() {
								...
								dataVal = '';
								teamurl = 'http://192.168.8.31:8088/WhenWhere/home/team?teamNum='
										+ teamNum + '&teamname=' + teamName;
								console.log(teamurl);

								var span = $(this).find('#spanTag span')
										.clone();

								$('#a_sts')
										.text($(this).find('#status').text());
								$('#a_no').text();
								$('#a_url').text(teamurl);
								$('#a_type').html(span);
								$('h3.profile-username').text(
												...
												success : function(data) {
													...
													if (data[0].ok) {
														...
														for (var i = 1; i < data.length; i++) {
															...
															json_param
																	.push(data[i].nickName);
															json_param
																	.push(data[i].content);

															var msgObject = new msgObj(
																	json_param);
															str_Txt = msgObject
																	.msg_format();
															$(
			...
			switch (this.subject) {
	    	  ...
	    	  default :  typeName = '기타';
				...
				str += '<tr role="row" class="'+ class_no + ' ' + class_team + ' ' + class_sts + '\">'
						...
						+ this.teamname
						+ '</td>'
						+ '<td id="sdate" style="max-width: 20px;">'
						+ this.sdate
						+ '</td>'
						+ '<td id="edate" style="max-width: 20px;">'
						+ this.edate
						+ '</td>'
						+ '<td id="status" style="width: 80px;" class="hidden-xs">'
						+ status
						+ '</td>'
						+ '<td id="spanTag" style="width: 80px;" class="hidden-xs">'
						+ className + typeName + '</span></td></tr>';
				return str;
			}
		}
			...
			this.msg_format = function() {
				...
				if (this.nickName != sessionNick) {
					...
					str += '<div class="direct-chat-msg">'
							...
							+ '</span></div> '
							+ '<img class="direct-chat-img" src="../images/team/team2.PNG" alt="Message User Image">'
							+ '<div class="direct-chat-text">' + this.content
							+ '</div></div>';
				} else {
					str = '';
		...
		function applyUserDelete() {
			...
			$('.applyUserDeleteBtn')
					.on(
							'click',
							function() {
								$('.applyCheck:checked').each(
										function(k, v) {
											deleteUser[k] = {};
											deleteUser[k].tNo = $(this).val();
											deleteUser[k].nickname = $(this)
													.parent().next('#nickname')
													.text();
										});
		...
		function applyUser() {
			...
			$(document).on(
					...
					'.applyBtn',
					function() {
						nickname = $(this).parent().siblings('#nickname')
								.text();
						tNo = $(this).parent().siblings('#check').children()
								.val();
						$.ajax({
							type : "POST",
							url : "../team/applyUser",
						...
						success : function(data) {
							...
							for (var i = 0; i < data.length; i++) {
								...
								str = '<tr role="row" class="">'
										...
										+ '<td id="nickname" style="max-width: 40px;">'
										+ data[i].nickname
										+ '</td>'
